...
contract ClaimAirdrop is Script {
    ...
    error Interact__InvalidScriptSignature();

    function chaimAirdrop(address airdrop) public {
        vm.startBroadcast();
    ...
    function splitSignature(
        ...
        if (sig.length != 65) {
            revert Interact__InvalidScriptSignature();
        }
        assembly {
            r := mload(add(sig, 32))
            s := mload(add(sig, 64))
    ...
    function run() external {
        ...
        chaimAirdrop(mostRecentlyDeployed);
    }
}
